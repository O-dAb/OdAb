        ...
        <Card className="border-0 shadow-xl rounded-2xl bg-gradient-to-r from-green-100 via-yellow-50 to-green-50 dark:from-green-900 dark:via-yellow-900 dark:to-green-900">
            ...
            {todayReviews.length > 0 ? (
              <div className="space-y-3">
                {todayReviews.map((review, index) => (
                  ...
                  >
                    <div>
                      <div className="font-medium text-green-700 dark:text-green-300">{review.topic}</div>
                      <div className="text-sm text-gray-500 dark:text-gray-400">
                        {review.stage} 복습
                      </div>
                    </div>
                    <Badge className="bg-green-400 dark:bg-green-600 rounded-lg px-3 py-1">{review.stage}</Badge>
                  </div>
                ))}
              </div>
            ) : (
          ...
          <CardFooter className="pt-2 pb-6">
            <Link href="/review" className="w-full">
              <Button
                className="w-full bg-green-400 hover:bg-green-500 dark:bg-green-700 dark:hover:bg-green-600 rounded-xl font-medium shadow-sm"
              >
                복습하기
              </Button>
            </Link>
        ...
        <Card className="border-0 shadow-xl rounded-2xl bg-gradient-to-r from-blue-50 via-purple-50 to-blue-100 dark:from-blue-900 dark:via-purple-900 dark:to-blue-900">
          ...
          <CardContent className="pt-6">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <div className="text-sm font-medium text-blue-700 dark:text-blue-300">최근 학습</div>
                <Badge
                  className="bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300 rounded-lg px-3 py-1"
                >
                  오늘
                </Badge>
              </div>
              ...
              <div className="space-y-2">
                {recentTopics.length > 0 ? (
                  recentTopics.map((topic, index) => (
                    ...
                    >
                      <div className="text-blue-700 dark:text-blue-300">{topic}</div>
                    </div>
                  ))
                ) : (
          ...
          <CardFooter className="pt-2 pb-6">
            <Link href="/solve" className="w-full">
              <Button
                className="w-full bg-blue-400 hover:bg-blue-500 dark:bg-blue-700 dark:hover:bg-blue-600 rounded-xl font-medium shadow-sm"
              >
                문제 풀기
              </Button>
            </Link>
