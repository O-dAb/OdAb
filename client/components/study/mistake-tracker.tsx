...
export function MistakeTracker({ educationLevel, grade }: MistakeTrackerProps) {
  ...
  const [recentTabQuestions, setRecentTabQuestions] = useState<WrongQuestionDto[]>([]);


  // 개념 관련 상태
  const [majorConcepts, setMajorConcepts] = useState<WrongQuestionMajorConcept[]>([]);
  const [selectedMajorConcept, setSelectedMajorConcept] = useState<number | null>(null);
  ...
  return (
    <div className="space-y-8 bg-gradient-to-br from-pink-50 via-blue-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 min-h-screen p-6">
          ...
          <CardTitle className="flex justify-between items-center text-blue-700 dark:text-blue-300">
            ...
            <div className="flex items-center gap-2">
              <Select
                ...
                disabled={loading}
              >
                <SelectTrigger className="w-[100px] bg-blue-50 dark:bg-gray-800 border-blue-100 dark:border-gray-600 rounded-xl dark:text-gray-200">
      ...
      >
          <TabsList className="grid grid-cols-2 mb-6 bg-blue-100 dark:bg-gray-700 rounded-xl shadow">
            <TabsTrigger
              value="all"
              className="flex items-center gap-2 data-[state=active]:bg-blue-400 dark:data-[state=active]:bg-blue-600 data-[state=active]:text-white rounded-xl font-bold dark:text-gray-300"
            >
          ...
          <TabsTrigger
            value="recent"
            className="flex items-center gap-2 data-[state=active]:bg-yellow-400 dark:data-[state=active]:bg-yellow-600 data-[state=active]:text-white rounded-xl font-bold dark:text-gray-300"
            >
        ...
        {activeTab === "all" && (
          <div className="space-y-4">
            {/* 대주제 필터 */}
            <div>
              <div className="text-sm font-medium dark:text-gray-300 mb-2">대주제 필터</div>
                ...
                <Button
                  ...
                  className={
                    selectedMajorConcept === null
                      ? "bg-purple-400 hover:bg-purple-500 dark:bg-purple-600 dark:hover:bg-purple-500 rounded-xl font-bold"
                      : "border-purple-100 dark:border-gray-600 text-purple-500 dark:text-purple-300 hover:bg-purple-50 dark:hover:bg-gray-700/50 rounded-xl font-bold"
                  }
                  disabled={loading}
                >
                  전체
            ...
            {selectedMajorConcept !== null && subconcepts.length > 0 && (
              <div>
                <div className="text-sm font-medium dark:text-gray-300 mb-2">소주제 필터</div>
                <div className="flex flex-wrap gap-2">
                  {subconcepts.map((subconcept) => (
                    <Button
                      ...
                      className={
                        selectedTopic === subconcept.subConceptId
                          ? "bg-blue-400 hover:bg-blue-500 dark:bg-blue-600 dark:hover:bg-blue-500 rounded-xl font-bold"
                          : "border-blue-100 dark:border-gray-600 text-blue-500 dark:text-blue-300 hover:bg-blue-50 dark:hover:bg-gray-700/50 rounded-xl font-bold"
                      }
                      disabled={loading}
                    >
            ...
            {loading ? (
              ...
              filteredQuestions.map((question) => (
                <Card
                  ...
                  className="border-0 shadow-lg rounded-2xl bg-gradient-to-r from-blue-50 via-purple-50 to-pink-50 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800"
                >
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        {question.questionImg && (
                          <img
                            ...
                            alt="문제 이미지"
                            className="mt-2 max-h-40 object-contain rounded-xl border border-blue-100 dark:border-gray-700"
                          />
                        )}
                      </div>
                      ...
                      {question.solvedOnRetry ? (
                        <Button
                          size="sm"
                          variant="outline"
                          className="flex items-center gap-1 border-blue-100 dark:border-gray-600 text-blue-500 dark:text-blue-300 hover:bg-blue-50 dark:hover:bg-gray-700/50 rounded-xl font-bold"
                          disabled={true}
                        >
        ...
        <TabsContent value="recent" className="mt-0">
          <div className="space-y-4">
            {loading ? (
              <Card className="border-yellow-100 dark:border-gray-700">
                <CardContent className="p-6 text-center">
                  <p className="text-gray-500 dark:text-gray-400">로딩 중...</p>
                </CardContent>
              </Card>
              ...
              filteredQuestions.map((question) => (
                <Card
                  key={question.questionId}
                  className="border-0 shadow-lg rounded-2xl bg-gradient-to-r from-yellow-50 via-pink-50 to-purple-50 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800"
                >
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          {/* 해당 문제의 주제 찾기 */}
                          <Badge className="bg-yellow-400 dark:bg-yellow-600 rounded-full px-3 py-1 text-white font-bold">
                            {question.wrongQuestionSubconceptList?.[0]
                              ?.subConceptType || "주제 없음"}
                          </Badge>
                          <span className="text-sm text-gray-500 dark:text-gray-400">
                        ...
                        {question.questionImg && (
                          <img
                            src={question.questionImg}
                            alt="문제 이미지"
                            className="mt-2 max-h-40 object-contain rounded-xl border border-yellow-100 dark:border-gray-700"
                          />
                        )}
                      </div>
                      ...
                      {question.solvedOnRetry ? (
                        <Button
                          size="sm"
                          variant="outline"
                          className="flex items-center gap-1 border-yellow-100 dark:border-gray-600 text-yellow-600 dark:text-yellow-300 hover:bg-yellow-50 dark:hover:bg-gray-700/50 rounded-xl font-bold"
                          disabled={true}
                        >
                          <RotateCcw className="h-3 w-3" />
                          정답 처리됨
