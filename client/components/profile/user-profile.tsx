...
export function UserProfile({ onProfileUpdate }: UserProfileProps) {
  ...
  const handleSaveProfile = () => {
    // 프로필 저장
    localStorage.setItem("userProfile", JSON.stringify({ level: educationLevel, grade }))
    setIsProfileSet(true)
    onProfileUpdate(educationLevel, grade)

    toast({
      title: "프로필이 저장되었습니다",
      description: `${educationLevel === "middle" ? "중학교" : "고등학교"} ${grade}학년으로 설정되었습니다.`,
    })
  }

  return (
    <Card className="border-blue-200">
      <CardHeader className={`${educationLevel === "middle" ? "bg-green-400" : "bg-blue-500"} text-white`}>
      ...
      <CardContent className="pt-6">
        <div className="space-y-6">
          <div className="space-y-4">
            ...
            <RadioGroup
              ...
              onValueChange={(value) => {
                ...
                if (header) {
                  header.className = value === "middle" ? "bg-green-500" : "bg-blue-500"
                }
              }}
          ...
          <Button onClick={handleSaveProfile} className="w-full bg-blue-500 hover:bg-blue-600">
